...
def _risks(option: dict, late_by: int, plan: dict) -> list[tuple[str, str]]:
    """把這個方案的具體風險列出來，不做成分數或機率。

    每一項是 (代碼, 說明)。代碼讓後面的重新規畫能區分「一定要換」（late、no_time）
    與「有更好的再換」（rain、parking_low），不必靠比對中文句子。
    """
    found = []
    # 還沒到出發時刻時 late_by 自然是 0，不需要額外用「課還很久」去壓抑警告——
    # 那樣反而會在人還在外縣市、路程長到已經來不及時把警告吃掉
    if late_by > 0:
        found.append(("late", f"已經超過最晚出發時間 {late_by} 分鐘"))
    if plan.get("weather_advice"):
        found.append(("rain", plan["weather_advice"]))
    lot = plan.get("parking")
    if lot and lot.get("available") is not None:
        if lot["available"] < PARKING_RISK_THRESHOLD:
            found.append(("parking_low",
                          f"{lot['name']}目前只剩 {lot['available']} 位，抵達時可能已滿"))
    if option["minutes"] is None:
        found.append(("no_time", "算不出路程時間"))
    return found


def build_option(mode: str, plan: dict, now: datetime, class_starts: datetime) -> dict:
    """把 plan_departure 對某一種交通方式的結果，整理成比較用的一筆選項。

    獨立成函式，是因為重新規畫只想「快速檢查目前這一種」時，不必把四種全跑一遍。
    """
    ...
    option = {
        ...
        # 算不出時間時 plan_departure 會說明原因（例如附近沒有借得到車的站）
        "note": plan.get("note", ""),
        "leave_by": None,
    ...
    pairs = _risks(option, option["late_by"] or 0, plan)
    option["risks"] = [text for _, text in pairs]
    option["risk_codes"] = [code for code, _ in pairs]
    return option


def compare_plans(origin: str = "", vehicle_type: str = "機車",
                  schedule_path: str = "", now: datetime | None = None) -> dict:
    ...
    Args:
        ...
        now: 用哪個時間當「現在」，留空是真實時間。網頁的模擬時間靠它，
            讓「跳到上課前 20 分」時各方案的最晚出發與遲到判斷跟著變。
    ...
    plan_kwargs = {"now": now} if now is not None else {}
    now = now or datetime.now(ZoneInfo(settings.timezone))

    options, course, class_starts = [], None, None
    for mode in MODES:
        plan = plan_departure(origin, mode, vehicle_type, schedule_path, **plan_kwargs)
        if plan["status"] != "ok":
        ...
        options.append(build_option(mode, plan, now, class_starts))
