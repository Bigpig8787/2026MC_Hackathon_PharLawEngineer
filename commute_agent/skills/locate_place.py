...
3. 請 Gemini 把原文讀成「可以拿去搜尋的關鍵字」，再把關鍵字丟回 GIS 驗證。

第三層是這支的重點，設計上刻意讓模型只做它擅長的事：讀懂「社科院大樓階梯教室
...
def locate_course_place(location_text: str, room_query: str = "",
    ...
    Args:
        ...
        use_gemini: 前兩層都查不到時，要不要請 Gemini 幫忙讀這串文字。

    Returns:
        dict，包含：
        - status: "ok" 或 "not_found"
        - source: 答案是怎麼來的 —— "gis_room_code"（教室代碼查 GIS，最準）、
          "gis_text"（原文查 GIS）、"gemini_room" / "gemini_building"
        ...
        - hint: 走 Gemini 那條時，模型給的關鍵字與判讀說明
    """
    ...
    key = f"{text}|{room_query}|{use_gemini}"
    if key in _CACHE:
        return _CACHE[key]
